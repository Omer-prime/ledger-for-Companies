...
type Cat = { _id: string; name: string; code?: string }
...
export default function ProductCategoriesPage() {
  ...
  async function load(query = '') {
    const url = query ? `/api/product-categories?q=${encodeURIComponent(query)}` : '/api/product-categories'
    ...
    <main className="space-y-4">
      <div className="rounded-2xl bg-white p-6 shadow-soft">
        ...
        <form onSubmit={create} className="mt-4 grid gap-3 md:grid-cols-[1fr_200px_160px]">
          <input className="rounded-xl border px-3 py-2" placeholder="Category name" value={name} onChange={e=>setName(e.target.value)} required />
      ...
      <div className="rounded-2xl bg-white p-6 shadow-soft">
        <div className="mb-3 flex items-center gap-2">
          <input className="w-full rounded-xl border px-3 py-2" placeholder="Search categories…" value={q} onChange={e=>setQ(e.target.value)} />
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {list.map(c=>(
                <tr key={c._id} className="border-b/50">
